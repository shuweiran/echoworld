...
## 📊 当前基线（最新汇总，2026-08-02 22:1x）

| 指标 | 值 |
|---|---|
| 测试类 | **43** |
| 测试用例 | **309（302 基线 + P-0802-J 新增 7：ScriptGamePerGameIsolationTest 2 + WerewolfRoleKeyTest 4 + SSEControllerSessionTest +1）** |
| Failures / Errors | **0 / 0** |
| 最后全量执行 | 2026-08-02 22:1x（**P-0802-J 狼人杀范围外遗留项批次：全量 309/0 BUILD SUCCESS**，43 类 surefire 汇总；首轮 8 类定向跑 33/0 后全量复跑 309/0，无堆测量脆性波动，LONG-01 6.78s PASS；覆盖：剧本杀讨论引擎 per-game 隔离（I-1/I-2）+ script_* SSE 会话定向（P3）+ 狼人杀 resume roleKey 防冒充（R-1~R-4）） |
