...
## 📊 当前基线（最新汇总，2026-08-02 23:5x）

| 指标 | 值 |
|---|---|
| 测试类 | **51** |
| 测试用例 | **352（335 基线 + P-0802-M LLMClientStreamTest 7 + P-0802-P3 新增 10：PlayerRenameE2ETest 6 + ScriptRenameResumeTest 4）** |
| Failures / Errors | **0 / 0** |
| 最后全量执行 | 2026-08-02 23:5x（**P-0802-P3 改造方案 Phase 3 局中改名端点·同步式：全量 352/0 BUILD SUCCESS**，51 类 surefire 汇总，LONG-01 10 万字 2.758s PASS；覆盖：新 PlayerController POST /api/player/rename（200/403/409/500+rolled_back）→ PlayerIdentityService.renamePlayerCharacter 编排（撞名校验②→角色库改名→RouterService/2D/狼人杀/剧本杀四服务同步→失败回滚）→ 返回 synced_sessions 清单；RouterService.renameAgent 改动经主人授权 P-0802；**无 player_id / 未改名请求行为零变化**；前端零改动 → npm build 跳过） |
