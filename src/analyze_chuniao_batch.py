...
def main():
    ...
    # 分批 LLM 提取
    batch_size = 50
    total_batches = (len(meaningful) + batch_size - 1) // batch_size
    print(f'\n[2/2] LLM 分析 ({len(meaningful)} 个会话, {total_batches} 批)...')
    ...
    for batch_idx in range(total_batches):
        ...
        end = min(start + batch_size, len(meaningful))
        batch = meaningful[start:end]

        print(f'\n--- 批次 {batch_idx+1}/{total_batches} (会话 {start+1}-{end}) ---')
        ...
        print(f'  批次完成，发现 {len(batch_results)} 个有意义会话')

        # 保存中间结果
        with open(os.path.join(output_path, f'chuniao_batch_{batch_idx+1}.json'), 'w', encoding='utf-8') as f:
