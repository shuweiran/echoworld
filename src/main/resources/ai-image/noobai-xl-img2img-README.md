# noobai-xl-img2img-workflow.json — 表情 img2img 工作流模板说明（P-0810-05；基座已换 NoobAI-XL）
...
## 节点表（与文生图模板 noobai-xl-workflow.json 的差异：无 EmptyLatentImage(9)，改 LoadImage+VAEEncode）

| 节点 | class_type | 输入 | 说明 |
|---|---|---|---|
| 1 | CheckpointLoaderSimple | ckpt_name=NoobAI-XL-v1.1.safetensors | NoobAI-XL 二次元底模（model slot0 / clip slot1 / vae slot2） |
| 5 | LoraLoader | model=[1,0] clip=[1,1] lora_name=__LORA_NAME__ strength 0.8 | 风格 LoRA（可选；默认空=正常二次元，lora 名为空时 ComfyUIClient.applyLoraRewiring 自动改接并移除本节点） |
| 13 | CLIPSetLastLayer | clip=[5,1] stop_at_clip_layer=-2 | SDXL 系 clip skip 2（新版 ComfyUI 0.31 要求） |
| 7 | CLIPTextEncode | clip=[13,0] text=__POSITIVE__ | 正向提示词（质量 tag + 风格 + 外貌 + 表情） |
...
| 10 | KSampler | model=[5,0] positive=[7,0] negative=[8,0] latent_image=[F,0] seed=__SEED__ steps=28 cfg=5.5 euler_ancestral normal **denoise=__DENOISE__** | 采样；denoise 为 img2img 强度（默认 0.5，可配 roleplay.ai-image.img2img-denoise；0.45 实测脸型/发型/服装全保持） |
