/* CDP 验证：2D 探索 = 瓦片背景 + 双主控动态 + 玩家角色（SimulationScene） */
...
const OUT = 'D:/roleplay-java/tmp/explore_sim_verify';
...
async function main(){
  const child = spawn(EDGE, ['--headless=new','--no-proxy-server','--disable-gpu','--no-sandbox',
    ...
    '--user-data-dir=C:\\Temp\\edge-sim-'+Date.now(),'about:blank'], { stdio:'ignore', detached:true });
  ...
  // 等 PhaserSimulationView（动态模拟）挂载 —— 2D 模拟标识
  await waitFor(cdp,`!!document.querySelector('.phaser-sim-view')`,120000,'sim-view');
  await waitFor(cdp,`!!document.querySelector('.phaser-sim-view canvas')`,60000,'sim-canvas');
  log('=== 动态模拟视图挂载 ===');
  await sleep(3000);
  // 检查控制条 + 聊天面板 + SSE 状态
  const info = await cdp.eval(`(()=>{
    const view=document.querySelector('.phaser-sim-view');
    return JSON.stringify({
      hasCanvas: !!document.querySelector('.phaser-sim-view canvas'),
      hasChat: !!document.querySelector('.sim-chat-panel'),
      hasStart: Array.from(document.querySelectorAll('button')).some(b=>b.textContent.includes('开始')),
      status: (Array.from(document.querySelectorAll('.phaser-sim-view span')).map(s=>s.textContent).find(t=>t&&t.includes('SSE'))||'')||(Array.from(document.querySelectorAll('.phaser-sim-view span')).map(s=>s.textContent).find(t=>t&&t.includes('已连接'))||''),
    });
  })()`);
  log('[sim-info]', info);
  const o = JSON.parse(info);
  check('动态模拟 canvas 渲染', o.hasCanvas===true);
  check('聊天面板存在', o.hasChat===true);
  check('开始/暂停按钮存在', o.hasStart===true);
  await cdp.shot('sim_explore.png');
