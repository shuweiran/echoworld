...
const PROTO_PROBE = `(()=>{
  ...
  return {
    ...
    chip: gs('.chip:not(.on)',['border','background-color','color','border-radius','padding','font-size']),
    chipOn: gs('.chip.on',['background','color','box-shadow']),
    clue: gs('.clue:not(.new)',['background-image','border','border-radius','box-shadow','padding']),
...
const REAL_PROBE = `(phase)=>{
  ...
  return {
    ...
    avatar: M('.proto-char-avatar','<div class="proto-char"><span class="proto-char-avatar" style="background:linear-gradient(135deg,#38bdf8,#0284c7)">a</span></div>',['width','height','border-radius','font-size','font-weight','background-image','box-shadow']),
    ...
    bubble: M('.proto-msg-text','<div class="proto-discuss-stream"><div class="proto-msg-text" style="--msg-c:#ec4899">t</div></div>',['background-color','border-left-width','border-left-color','border-radius','padding','font-size','line-height','color']),
...
function compareObj(proto, real, map) {
  ...
  for (const [pKey, rKey] of map) {
    ...
    if (p == null || r == null) continue; // 一侧缺失（该页无此元素）→ 不对比，避免假阴性
...
async function main() {
  ...
  await sleep(500);
  // P-0816-U：选择玩家角色（否则 currentPlayer='我' 与对局 players 不匹配，行动条不渲染）
  const picked = await cdp.eval(`(()=>{
    const picker=[...document.querySelectorAll('.role-chip')].find(x=>x.textContent.includes('选择你的角色'));
    if(!picker) return 'NO-PICKER';
    picker.click(); return 'PICKED';
  })()`);
  log('player picker:', picked);
  await waitFor(cdp, `!!document.querySelector('.modal-title') && document.querySelector('.modal-title').textContent.includes('选择你的玩家角色')`, 10000, 'picker modal');
  await sleep(400);
  const pickedRole = await cdp.eval(`(()=>{
    // 优先「📜 本剧本角色」段；找不到就取弹窗内第一个非取消按钮的 role-chip
    const modal=document.querySelector('.modal-box'); if(!modal) return 'NO-MODAL';
    const chips=[...modal.querySelectorAll('.role-chip')];
    const target=chips.find(c=>c.textContent.includes('🔒'))||chips[0];
    if(!target) return 'NO-CHIP';
    target.click(); return target.textContent.slice(0,20);
  })()`);
  log('picked role:', pickedRole);
  await sleep(400);
  await cdp.eval(`(()=>{ const b=[...document.querySelectorAll('button')].find(x=>x.textContent.includes('进入对局')); if(b){ if(b.disabled){ const c=[...document.querySelectorAll('.role-chip')].find(x=>!x.textContent.includes('选择你的角色')&&!x.textContent.includes('✕')); if(c) c.click(); } setTimeout(()=>b.click(),50); } })()`);
  await waitFor(cdp, `!!document.querySelector('.workspace.proto-v2') && !!document.querySelector('.proto-topbar')`, 45000, 'proto-v2 page');
  // P-0816-U：若停在 SETUP（LLM 完整剧本异步生成慢），REST 强制触发 generate_full 兜底
  const badgeOk = await waitFor(cdp, `(()=>{ const b=document.querySelector('.proto-badge-phase'); return b && b.textContent.includes('搜证'); })()`, 25000, 'investigation').then(() => true).catch(() => false);
  if (!badgeOk) {
    log('未在 25s 内到搜证 → REST 触发 generate_full 兜底');
    try {
      const g = await restPost('/api/script/generate_full', {});
      log('generate_full resp:', JSON.stringify(g).slice(0, 160));
    } catch (e) { log('generate_full err:', e.message.slice(0, 120)); }
    await waitFor(cdp, `(()=>{ const b=document.querySelector('.proto-badge-phase'); return b && b.textContent.includes('搜证'); })()`, 120000, 'investigation after gen').catch(()=>log('warn: 仍未到搜证'));
  }
  await sleep(3000);
  ...
  for (const ph of phases) {
    ...
    for (const [pk, rk] of Object.entries(elemMap)) {
      const rows = compareObj(p, r, [[pk, rk]]);
